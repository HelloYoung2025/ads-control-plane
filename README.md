...
2026-09-22 在 SFW **1.1.0**（构建 10015）真机上整条链路跑通过一次（假数据、同一账号、无 uid 隔离）：登记→连接→/fd→工具执行→回答逐字一致。权限隔离那部分仍未在真机验过，首次安装时逐步核对，对不上就停。
**必须把对话档位设成「完全访问权限」。** 1.1.0 的「请求批准」档下，MCP 工具调用会弹一个「MCP 请求你的输入」窗，窗上写着「此请求当前只允许拒绝」——点提交无反应，等满 5 分钟超时，工具永远调不通，而且那个窗会把整个界面锁住到重启 SFW。这不是可选项，是 1.1.0 唯一能用的档位。
**宿主会自动升级**：本机 2026-09-22 19:33 从 1.0.8 静默升到 1.1.0（设置→通用→应用更新可关）。下面这些结论只对实测过的版本成立，升级后要重验第 1 次 `/fd`。
...
10. 先把输入框左下角的档位设成「完全访问权限」（见开头那段，「请求批准」档下工具调不通）。培训两句话：① 敲 `/new` 回车，再敲 `/fd` 回车回车；② 文件在回答里的蓝字链接和桌面「否定词导出」里，报表点开看。**不会弹任何窗**；真弹出「MCP 请求你的输入」就是档位被改回去了，叫管理员。
