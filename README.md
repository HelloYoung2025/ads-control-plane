...
1. **打开「终端」，把下面两行粘进去回车**（只有第一次需要，不用管理员密码，也不用先有 Python）。
   第一行装 uv，第二行把组件先下下来——第二行跑完不报错，就说明这台机器没问题了。
   ...
   ```bash
   uvx --from git+https://github.com/HelloYoung2025/ads-control-plane@v0.1.0 amazon-ads --help
   ```

   第二行第一次要两三分钟（2026-09-22 实测 159 秒），之后就有缓存了。跳过它也能用，
   只是第一次在 SFW 里问话时要在那儿干等这段时间。

2. **在 SFW 里加来源**：左边「定制化」→「插件」→ 展开最底下的「高级 · 插件来源」→
   在输入框里粘下面这一行 → 点「添加」。

   ```
   HelloYoung2025/ads-control-plane@v0.1.0
   ```

   末尾那个 `@v0.1.0` 不能省：不带版本号会去仓库的默认分支找，那上面没有这套东西
   （2026-09-22 实测，报「marketplace root does not contain a supported manifest」）。
...
要是跳过了上面第 1 步的第二行，第一次问会先卡两三分钟把组件装好；之后一家店通常半分钟内
...
从公开地址也整条走过一遍：`HelloYoung2025/ads-control-plane@v0.1.0` → 装上 → 启动脚本从 GitHub
拉到 tag 指向的那个提交、编译、装好 29 个依赖 → 在一个全新的家目录里写出配置模板 → stdio 待命。
**冷启动 28 秒**（所以 180 秒的超时是够的）。

**还没验**：在 SFW 界面里点着装（上面是用它自带的引擎在隔离环境里验的）、接真实领星数据、
以及上面那条 `.pkg` 的路。
