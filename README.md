...
**第二块**：整块一起粘进去回车。中间两行把组件装进 SFW，最后一行把它先下下来。

```bash
export CODEX_HOME=~/.sfw/engine-home
C=/Applications/SFW.app/Contents/Frameworks/vendor/bin/codex
"$C" plugin marketplace add HelloYoung2025/ads-control-plane@v0.1.0
...
上面那两行命令调的就是界面底下的同一个引擎，所以这里给命令而不是给点击步骤。
...
**还没验**：接真实领星数据；上面那条 `.pkg` 的路（从没在任何机器上装过）。
