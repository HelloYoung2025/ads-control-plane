...
"$C" plugin marketplace add HelloYoung2025/ads-control-plane@v0.1.2
"$C" plugin add amazon-ads@ecom-pack
uvx --from git+https://github.com/HelloYoung2025/ads-control-plane@v0.1.2 amazon-ads --help
```
...
两处 `@v0.1.2` 别删：它钉住你装的是哪一版，插件自己启动时拉的也是这一版。只换版本号、
不先挪掉旧的，会报「already added from a different source」（2026-09-23 实测），所以第三行不能省。

装完（以后升级也一样）把 SFW **完全退出（⌘Q）再打开**，然后开一个新对话用。只开新对话不够：
SFW 的引擎要等 SFW 退出才重新读插件。2026-09-24 实测，升级后没退出 SFW，新对话拉起的仍是旧版，
模型还去找已经删掉的旧文件；⌘Q 再打开之后才是新版。
...
- 助手说「工具没连上」：SFW「定制化 →插件」里看这个插件还在不在、启用没有；再 ⌘Q 退出 SFW、重新打开，开一个新对话。
- 助手念出「配置错误：…」：那句话里就写着要改哪个文件、改什么，照着做完重开对话。
- 界面上这个 MCP 服务器起不来：多半是找不到 uv。把第一块重跑一次，然后 ⌘Q 退出 SFW 再打开。
...
看有没有比你装的更新的版本号。有，就把「装」第二块里两处版本号换成它，整块重粘一遍，再 ⌘Q 退出 SFW 重新打开；
